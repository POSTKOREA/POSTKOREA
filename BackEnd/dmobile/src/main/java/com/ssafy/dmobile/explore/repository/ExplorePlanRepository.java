...
public interface ExplorePlanRepository extends JpaRepository<ExplorePlan, Long> {
    ...
    @Query("select e from ExplorePlan e where e.userId = :id and e.planStartDate <= :now and e.planEndDate >= :now")
    List<ExplorePlan> findOngoingPlans(Long id, Date now);
    @Query("select e from ExplorePlan e where e.userId = :id and e.planStartDate > :now")
    List<ExplorePlan> findUpcomingPlans(Long id, Date now);
    @Query("select e from ExplorePlan e where e.userId = :id and e.planEndDate < :now")
